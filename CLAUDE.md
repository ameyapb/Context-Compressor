...
- **[src/filterPanelProvider.js](src/filterPanelProvider.js)** — `FilterPanelProvider` for the Line Filter sidebar section. Constructor accepts `getContextLines` and `getFilterHistory` callbacks; `getFilterHistory()` returns the session-scoped filter history array maintained in `extension.js`. Shows two collapsible groups: Filter Summary (shows a persistent list of all filter iterations as collapsible `FilterHistoryGroupItem` entries, newest first; each entry label is the filter chain joined with ` > ` and its description shows the match count; clicking an entry opens or reopens that filter result via `vscode.open`; expanding reveals a `FilterSourceItem` for the original file plus chain steps and match count as `FilterInfoItem`s; shows "No filter results yet" when history is empty) and Actions (four clickable items: "Keep matching lines...", "Remove matching lines...", a selection-aware "From selection" item that shows a preview of the current selection, and a "Context: N lines" item that fires `setContextLines`). Exports `FilterPanelProvider`, `buildFilterState(firstLine)`, `FilterSourceItem`, `CONTEXT_VALUE_FILTER_SOURCE`, `FilterHistoryGroupItem`, and `CONTEXT_VALUE_FILTER_HISTORY_GROUP`. `FilterPanelProvider.refresh()` is called from `extension.js` after any filter command completes, on every active editor change, on every text-selection change (debounced 150ms), and when the history resets (all filter tabs closed).
