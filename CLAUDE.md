...
The extension is split across eight modules under `src/`:

- **[src/extension.js](src/extension.js)** — activation, command registration, status bar lifecycle. The only file that imports `vscode`. Registers ten commands: `countTokens`, `selectModel`, `countFolderTokens`, `addToContext`, `removeFromContext`, `clearContext`, `setCompressionMode`, `assemblePrompt`, `suggestRelatedFiles`, `managePresets`.
